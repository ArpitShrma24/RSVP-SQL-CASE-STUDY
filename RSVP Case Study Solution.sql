...
SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 else 0 END) AS wolrwide_gross_income_null,
...
/* Columns having null values are ["country"],["worlwide_gross_income"],["languages"],["production_company"].*/
...
/* YEAR 2017 HAS [3052] MOVIES, 2018 HAS [2944] MOVIES, 2019 HAS [2001] MOVIES */
...
/* LOOKING AT THIS TREND MONTH WISE WE FOUND THAT MAXIMUM NUMBER OF MOVIES ARE RELEASED IN THE MONTH OF "MARCH" */
...
SELECT  COUNT(DISTINCT id) AS count_movies, year 
FROM movie
WHERE (country LIKE '%USA%' OR country LIKE '%India%') AND year="2019";


/* USA AND INDIA COLLECTIVELY PRODUCED [1059] NUMBER OF MOVIES IN THE YEAR 2019 */
...
/* DRAMA HAS PRODUCED MAXIMUM NUMBER OF MOVIES OVERALL*/
...
WITH genre_information AS
(
SELECT genre, COUNT(movie_id) AS movie_count, 
RANK() OVER( ORDER BY COUNT(movie_id) DESC) AS genre_rank
FROM genre
GROUP BY genre )
SELECT * FROM genre_information 
...
-- Selecting the top 5 movies from each of the top 3 genres for each year based on worldwide gross income.
Final_result AS
(
SELECT g.genre AS genre,m.year AS year,m.title AS title,worlwide_gross_income,
...
ON m.id = r.movie_id
WHERE median_rating >= 8
AND POSITION(',' in languages) > 0 -- Movies available in different languages
...
SELECT name AS actress_name,SUM(total_votes) AS total_votes,COUNT(m.id) AS movie_count,AVG(avg_rating) AS actress_Avg_rating,
...
--------------------------------------------------------------------------------------------*/
-- Type you code below:
-- Initially creating a code for calculating the average difference between two movie dates.
...
SELECT director_id, director_name, AVG(DATEDIFF(next_movie_date, movie_date)) AS avg_inter_movie_days
FROM movie_dates
...
SUM(m.duration) AS total_duration,ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT dm.movie_id) DESC) AS director_rank
FROM names AS nm
INNER JOIN director_mapping AS dm 
ON nm.id = dm.name_id
INNER JOIN movie AS m 
...
SELECT
td.director_id, td.director_name,td.number_of_movies,
